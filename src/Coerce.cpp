  ...
  void process(const ProcessArgs &args) override
  {
    for (int i = 0; i < INPUTS_AND_OUTPUTS; i++)
    {
      if (inputs[IN1_INPUT + i].isConnected() && outputs[OUT1_OUTPUT + i].isConnected())
      {
        ...
        {
          adjustValues(sel_id, inputs[IN1_INPUT + i].getVoltages(), outputs[OUT1_OUTPUT + i].getVoltages(), inputs[IN1_INPUT + i].getChannels(), selections, sel_count);
        }
        else
        {
...
struct RestrictMethodMenuItem : MenuItem
{
  ...
  void onAction(const event::Action &e) override { module->restrictMethod = method; }
  void step() override
  {
...
struct RoundingMethodMenuItem : MenuItem
{
  ...
  void onAction(const event::Action &e) override { module->roundingMethod = method; }
  void step() override
  {
...
template <typename BASE, int PORTS, const char *SVG> struct CoerceWidget : ModuleWidget
{
