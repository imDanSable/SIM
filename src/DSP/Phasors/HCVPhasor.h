...
class HCVPhasorBase {
   public:
    ...
    {
        frozenMult = _isFrozen ? 0.0f : 1.0f;
    }
    void setOutputScalar(float _scalar)
    {
        outputScalar = _scalar;
    }
    void setPulseWidth(float _pulseWidth)
    {
        pulseWidth = _pulseWidth;
    }
    void setJitterDepth(float _jitterDepth)
    {
        jitterDepth = _jitterDepth;
    }
    void setPulsesPerCycle(int _pulsesPerCycle)
    {
        pulsesPerCycle = _pulsesPerCycle;
    }
    float getJitterSample()
    {
        return jitterValue;
    }
    ...
    {
        if (phasorFinishedThisSample()) { jitterValue = randomGen.whiteNoise(); }
    }
...
class HCVPhasor : public HCVPhasorBase {
   public:
    ...
    float getCurrentPhase() override
    {
        return phasor.phase();
    }
    void setFreqDirect(float _freq) override
    {
        phasor.freq(_freq);
    }
    ...
    {
        reverseMult = _isReversed ? -1.0f : 1.0f;
    }
    bool phasorFinishedThisSample() override
    {
        return phasor.cycled();
    }
...
struct HCVBurst  // modified Gamma NShot to add stopBurst
{
    ...
    {
        return (mCount >= mRepeats) && (pos == 0xffffffff);
    }
    ...
    T operator()(T v, T inc, T max, T min)
    {
        ...
        return mCount < mRepeats ? gam::scl::wrap(v, max, min)
                                 : gam::phsInc::incClip(v, inc, max, min);
    }

    /// Set number of repetitions
    HCVBurst& repeats(uint32_t v)
    {
        mRepeats = v;
        return *this;
    }
    HCVBurst& number(uint32_t v)
    {
        mRepeats = v;
        return *this;
    }
